...
vi.mock("../ToolInvocationDisplay", () => ({
  ToolInvocationDisplay: ({ toolInvocation }: { toolInvocation: { toolName: string; args?: Record<string, unknown> } }) => {
    const fileName = typeof toolInvocation.args?.path === "string"
      ? toolInvocation.args.path.split("/").pop()
      : "";
    const command = toolInvocation.args?.command as string | undefined;
    let label = toolInvocation.toolName;
    if (toolInvocation.toolName === "str_replace_editor" && command === "create") {
